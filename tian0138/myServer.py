...
# binary
binary_type_files = set(["jpg", "jpeg","png","mp3","html","css","PNG"])
...
class ResponseBuilder:
    ...
    def build(self) -> bytes:
        ...
        #     response+=f"{header}"
        # response+=+NEWLINE+f"{self.content}"
        # return response
        r = f"{self.status}{NEWLINE}"
        for header in self.headers:
            r += f"{header}{NEWLINE}"
        r +=f"{NEWLINE}"
        r = r.encode('utf-8')
...
class HTTPServer:
    ...
    def get_request(self, request: Request) -> Response:
        ...
        if path_components[0]=="redirect":
            search_source=path_components[1].split("&")
            query = search_source[0].split("=")[1]
            source = search_source[1].split("=")[1]
            if source == "Youtube":
                header = "https://www."+source+".com/results?search_query="+query
            if source == "Google":
                header = "https://www."+source+".com/search?q="+query
            response=ResponseBuilder()
            response.add_header("Content-Type",'text/html')
            response.set_status("307","Temporary Redirect")
            response.add_header("Location",header)
        ...
        if not has_permission_other(request.path):
            ...
            response.add_header("Content-Type",'text/html')
            response.set_content(get_file_contents("403.html"))
            return response.build()
        if should_return_binary(path_components[1]):
            file_content=get_file_binary_contents(request.path)
        else:
            file_content=get_file_contents(request.path)
        response=ResponseBuilder()
        response.set_status("200","OK")
        response.add_header("Content-Type",get_file_mime_type(path_components[1]))
    ...
    def post_request(self, request: Request) -> Response:
        ...
        """
        decoded_content =unquote_plus(request.content)
        ...
        html = """<!DOCTYPE	html><head>
            ...
            <link rel="stylesheet" href="resources/css/Result.css"></head><body><table>"""
        for key,value in data.items():
            html+="<tr><th>{}</th><th>{}</th></tr>".format(key,value)
    ...
    def resource_not_found(self) -> Response:
        ...
        response.set_status("404","NOT FOUND")
        response.add_header("Content-Type",'text/html')
        response.set_content(get_file_contents("404.html"))
        return response.build()
